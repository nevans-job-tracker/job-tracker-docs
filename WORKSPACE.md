...
└── chrome-extension-job-tracker-close-posting/ <- manual closure extension
```
...
| [chrome-extension-job-tracker-close-posting](https://github.com/nevans-job-tracker/chrome-extension-job-tracker-close-posting) | Separate Chrome extension: closes an existing posting by exact URL. Published on `main`. |
...
**Complete — deployed and manually confirmed (KAN-83, 2026-09-11):** a separate
...
extension, tests, and fifth Git repository exist. The extension's sanitized
`main` was published and verified at `3ba772c` on 2026-09-11; its local configured
manifest stays ignored. The owner's terminal screenshot confirms manual backend
deployment at `16398b1`, with 304 tests passing, service running, health OK, and
the closure endpoint present. The owner then tested the extension manually and
confirmed it worked. No additional action is required for the first version;
the reported success is not an exhaustive live test of all sites/error cases.
Automatic checking and batching remain deferred.
